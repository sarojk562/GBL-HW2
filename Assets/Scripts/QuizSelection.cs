...
public class QuizSelection : MonoBehaviour
{
    ...
    public static string image = "Science";
    ...
    private void Start()
    {
        ...
        questionType.onValueChanged.AddListener(QuestionSelected);
    }

    private void QuestionSelected(int index)
    {
        question = "Generate any easy MCQ question related to " + questionType.options[index].text + ". Display options by line break and correct option should always be a) option and the remaining 3 options incorrect e.g. for b) , c) and d) generate incorrect answers.";
